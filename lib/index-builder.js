 ...
 */
export function buildIndex(dir, onProgress = () => {}) {
  const files = listMapFiles(dir);
  const ready = [];
  const skipped = [];

  files.forEach((file, i) => {
    onProgress('открываю  ', i + 1, files.length, path.basename(file));
  ...
  ready.forEach((entry, fileIndex) => {
    onProgress('индексирую', fileIndex + 1, ready.length, path.basename(entry.file));
 ...
 *
 * Сборка с холодного HDD стоит примерно 20 секунд на файл: на каждый нужен
 * проход по низким зумам таблицы tiles, а головка при этом мечется. На полном
 * наборе это несколько минут — поэтому наружу отдаётся ход работы, иначе
 * первый запуск выглядит зависшим. Дальше выручает кэш: перезапуск мгновенный.
 */
export function loadIndex(dir, cacheFile, onProgress) {
  ...
  const index = buildIndex(dir, onProgress);
